...
![drf-tutorial](./img/static/fastapi-tutorial.png)
...
1.  简易版前端效果
![drf-tutorial](./img/static/preview01.png)
2.  API交互文档 - **Swagger UI**
![drf-tutorial](./img/static/preview02.png)
3.  第三章的接口
![drf-tutorial](./img/static/preview03.png)
4.  第四、五章的接口
![drf-tutorial](./img/static/preview04.png)
5.  接口的参数、返回的状态码、描述
![drf-tutorial](./img/static/preview05.png)
6.  接口认证的效果
![drf-tutorial](./img/static/preview06.png)
7.  第七、八章和`img`应用的接口
![drf-tutorial](./img/static/preview07.png)
8.  城市信息表
![drf-tutorial](./img/static/preview08.png)
9.  感染数据表
![drf-tutorial](./img/static/preview09.png)
...
![drf-tutorial](./img/static/chapter02.png)
...
官网[FastAPI ](https://fastapi.tiangolo.com/zh/),FastAPI的特性有以下几点：

- 基于开发标准，使用JSON Schema

  >JSON Schema是一种描述JSON数据格式的语言，它提供了一种清晰、易于理解的方式来描述你的数据结构。你可以使用JSON Schema来验证JSON数据，确保数据满足你的要求。

- 自动生成文档

- 更主流的Python，全部基于Python 3.6类型声明，可以使用Pydantic编写带有类型声明的标准Python

- 编辑器支持，整个框架都被设计得易于使用且直观，且都支持自动补全

- 验证，可以校验大部分甚至所有的Python数据类型，还可以校验外来类型

- 安全性及身份验证（OAuth2，也使用JWT tokens）

- 依赖注入

  >FastAPI有一个使用非常简单，但是非常强大的依赖注入系统
  >
...
>ASGI（Asynchronous Server Gateway Interface）是一个标准接口，位于网络协议服务和Python应用之间，能够处理多种通用协议类型，包括HTTP，HTTP2和WebSocket[2](https://blog.csdn.net/weixin_43064185/article/details/108614935)。它是WSGI的精神继承者，专为具有异步功能的Python Web服务器、框架和应用程序之间提供标准接口[5](https://cloud.tencent.com/developer/article/1601400)。

>Uvicorn是一个轻量级、高性能的ASGI服务器，它基于uvloop和httptools构建。Uvicorn是用Python编写的，专为异步框架和应用程序提供服务。它支持HTTP/1.1、WebSockets和HTTP/2协议，使开发者能够构建高性能的Web服务。Uvicorn的安装非常简单，可以通过pip进行安装，支持最小的依赖（纯Python依赖）或包含“Cython-based”依赖和其他“可选额外”依赖。
...
![drf-tutorial](./img/static/chapter03.png)
...
![drf-tutorial](./img/static/chapter04.png)
...
![drf-tutorial](./img/static/chapter05.png)
...
![drf-tutorial](./img/static/chapter06.png)
...
![drf-tutorial](./img/static/chapter07.png)
...
![drf-tutorial](./img/static/chapter08.png)
...
先说下 COVID-19 全球数据源的问题，对接 [https://img-tracker-api.herokuapp.com](https://img-tracker-api.herokuapp.com) 的数据源，后台任务的方式将数据更新到数据表，前端只需要一个点击按钮
